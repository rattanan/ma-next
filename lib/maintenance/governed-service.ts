...
import { db } from "../db";
import {
  assets, auditLogs, maintenanceNotifications, notificationEvents, notificationReviews, workOrderAssignments,
...
import type {
  ...
  notificationCloseSchema, notificationInformationResponseSchema, notificationSchema, operatorDecisionSchema,
...
export async function createWorkOrderFromNotification(notificationId: string, input: Omit<AssignmentInput, "reason"> & { title?: string; description?: string; priority?: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" }, actor: Actor, meta: RequestMeta) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (notification.status !== "APPROVED" && notification.status !== "IN_MAINTENANCE") throw new HttpError(409, "Work orders require an approved notification", "INVALID_NOTIFICATION_STATUS");
...
export async function decideCompletion(id: string, input: ManagerDecisionInput, actor: Actor, meta: RequestMeta) {
  const now = new Date(); return db.transaction(async (tx) => {
    ...
    else { const waiting = transitionWorkOrder(next, "REQUEST_OPERATOR_ACCEPTANCE", { actor }); await tx.update(workOrderRechecks).set({ status: "APPROVED", resolvedAt: now }).where(and(eq(workOrderRechecks.workOrderId, id), ne(workOrderRechecks.status, "APPROVED"))); await tx.update(workOrders).set({ status: waiting, managerApprovedAt: now, updatedAt: now, updatedBy: actor.id }).where(eq(workOrders.id, id)); if (order.notificationId) { const notification = (await tx.select().from(maintenanceNotifications).where(eq(maintenanceNotifications.id, order.notificationId)).limit(1))[0]; if (notification?.status === "IN_MAINTENANCE") { const siblings = await tx.select({ id: workOrders.id, status: workOrders.status }).from(workOrders).where(eq(workOrders.notificationId, order.notificationId)); if (siblings.every((item) => item.id === id || ["MANAGER_APPROVED", "WAITING_FOR_OPERATOR_ACCEPTANCE", "OPERATOR_ACCEPTED", "CLOSED"].includes(item.status))) { const nnext = transitionNotification(notification.status, "REQUEST_OPERATOR_ACCEPTANCE", { actor }); await tx.update(maintenanceNotifications).set({ status: nnext, updatedAt: now, updatedBy: actor.id }).where(eq(maintenanceNotifications.id, notification.id)); await tx.insert(notificationEvents).values(notificationEvent(notification.id, "OPERATOR_ACCEPTANCE_REQUESTED", actor, now, notification.status, nnext)); } } } }
