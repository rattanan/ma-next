...
import { getPurchaseApprovalDetail, listPurchaseApprovalItems } from "@/lib/purchasing/approval-engine";
...
export async function pendingApprovalCount(actor: AuthenticatedUser) {
  ...
  const purchasingCount = actor.permissions.includes("VIEW_APPROVAL_CENTER") ? (await listPurchaseApprovalItems(actor, { tab: "pending", page: 1, pageSize: 1000 })).stats.pending : 0;
  return maintenanceCount + inventoryCount + purchasingCount;
}
...
export async function listApprovals(query: ApprovalQuery, actor: AuthenticatedUser) {
  ...
  if (query.type === "PURCHASE_REQUEST" || query.type === "PURCHASE_ORDER") return listPurchaseApprovalItems(actor, { tab: query.tab, search: query.search, type: query.type, page: query.page, pageSize: query.pageSize });
  ...
  const purchasing = actor.permissions.includes("VIEW_APPROVAL_CENTER") ? await listPurchaseApprovalItems(actor, { tab: query.tab, search: query.search, page: 1, pageSize: 1000 }) : { items: [], stats: { pending: 0, inReview: 0, overdue: 0, approvedToday: 0 } };
  const merged = [...filtered.map((task) => ({ ...task, requestedByName: requesterMap.get(task.requestedById) ?? "Unknown user", waitingMinutes: Math.max(0, Math.floor((Date.now() - task.requestedAt.getTime()) / 60000)) })), ...inventory.items, ...purchasing.items];
  ...
  return { items: sorted.slice(start, start + query.pageSize), total: sorted.length, page: query.page, pageSize: query.pageSize, pages: Math.max(1, Math.ceil(sorted.length / query.pageSize)), stats: { pending: accessible.filter((task) => task.status === "PENDING").length + inventory.stats.pending + purchasing.stats.pending, inReview: accessible.filter((task) => task.status === "IN_REVIEW").length + inventory.stats.inReview + purchasing.stats.inReview, overdue: accessible.filter((task) => activeStatuses.includes(task.status as (typeof activeStatuses)[number]) && Date.now() - task.requestedAt.getTime() >= 86_400_000).length + inventory.stats.overdue + purchasing.stats.overdue, approvedToday: accessible.filter((task) => task.status === "APPROVED" && task.completedAt && task.completedAt >= today).length + inventory.stats.approvedToday + purchasing.stats.approvedToday } };
}
...
export async function getApprovalDetail(id: string, actor: AuthenticatedUser) {
  ...
  if (!task) return (await getPurchaseApprovalDetail(id, actor)) ?? getInventoryApprovalDetail(id, actor);
