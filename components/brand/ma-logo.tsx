...
export function MaLogo({ className, compact = false, inverse = false, size = "md" }: BrandLogoProps) {
  ...
  return (
    <span className={cn("inline-flex shrink-0 items-center gap-3", className)} aria-label="MA Next Maintenance Intelligence">
      ...
      {!compact && (
        <span className="min-w-0 leading-none">
          ...
          <small className={cn("mt-1 block whitespace-nowrap font-medium tracking-[.04em]", scale.subtitle, inverse ? "text-blue-100/75" : "text-slate-500")}>Maintenance Intelligence</small>
        </span>
      )}
    </span>
  );
