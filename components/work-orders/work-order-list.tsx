...
export default function WorkOrderList({ permissions }: { permissions: string[] }) {
  ...
  return <main className="mx-auto max-w-[96rem] space-y-5 p-4 md:p-6">
    ...
    {loading ? <div className="space-y-3" aria-label="Loading work orders"><Skeleton className="h-20 rounded-xl" /><Skeleton className="h-20 rounded-xl" /><Skeleton className="h-20 rounded-xl" /></div> : data.items.length === 0 ? <Empty canCreate={permissions.includes("MANAGE_WORK_ORDERS")} /> : view === "list" ? <ListView items={data.items} /> : view === "board" ? <div className="flex snap-x gap-4 overflow-x-auto pb-3" aria-label="Work order status board">{board.map((column) => <section key={column.status} className="w-[17rem] shrink-0 snap-start rounded-xl border bg-slate-100/70 p-3 sm:w-72"><div className="mb-3 flex items-center justify-between gap-2"><Pill value={column.status} /><span className="shrink-0 text-sm font-bold">{column.items.length}</span></div><div className="space-y-2">{column.items.map((item) => <OrderCard key={item.id} item={item} />)}{column.items.length === 0 && <p className="rounded-lg border border-dashed p-4 text-center text-xs text-slate-500">No work</p>}</div></section>)}</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{calendar.map(([day, items]) => <section key={day} className="rounded-xl border bg-white p-4"><h2 className="font-bold">{day === "Unscheduled" ? day : new Date(`${day}T00:00:00`).toLocaleDateString([], { dateStyle: "full" })}</h2><div className="mt-3 space-y-2">{items?.map((item) => <OrderCard key={item.id} item={item} />)}</div></section>)}</div>}
...
function OrderCard({ item }: { item: Order }) { return <Link href={`/work-orders/${item.id}`} className="block min-w-0 overflow-hidden rounded-lg border bg-white p-3 shadow-sm transition hover:border-blue-300 hover:shadow"><div className="flex min-w-0 items-start justify-between gap-2"><strong className="min-w-0 truncate text-sm text-blue-800">{item.code}</strong><span className="shrink-0"><Pill value={item.priority} /></span></div><h3 className="mt-2 line-clamp-2 break-words font-semibold">{item.title}</h3><p className="mt-1 truncate text-xs text-slate-500" title={`${item.assetCode} · ${item.assetName}`}>{item.assetCode} · {item.assetName}</p><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><Pill value={item.status} /><time className={cn("shrink-0 text-xs", isOverdue(item) && "font-bold text-red-700")}>{date(item.dueAt)}</time></div></Link>; }
