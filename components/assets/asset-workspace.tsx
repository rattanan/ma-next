...
import {
  AlertCircle, Archive, ChevronDown, ChevronRight, FileText, Filter, FolderTree,
  ImageIcon, MapPin, Menu, PackageSearch, Pencil, Plus, RefreshCw, Search, ShieldX, Wrench,
} from "lucide-react";
...
function AssetDetail({ detail, allAssets, tab, onTab, permissions }: { detail: DetailPayload; allAssets: AssetNode[]; tab: Tab; onTab: (tab: Tab) => void; permissions: string[] }) {
  const [confirmArchive, setConfirmArchive] = useState(false); const [archiving, setArchiving] = useState(false); const [archiveError, setArchiveError] = useState("");
  async function archive() {
    setArchiving(true); setArchiveError("");
    try { const response = await fetch(`/api/assets/${detail.asset.id}`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ reason: "Archived from Asset Management detail" }) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || "Unable to archive asset"); window.location.reload(); }
    catch (error) { setArchiveError(error instanceof Error ? error.message : "Unable to archive asset"); setConfirmArchive(false); }
    finally { setArchiving(false); }
  }
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-2xl border bg-gradient-to-br from-[#0b2a4a] to-[#155aa0] text-white shadow-lg"><div className="grid gap-4 p-5 md:grid-cols-[9rem_1fr_auto] md:items-center md:p-6"><div className="hidden md:block"><AssetImage detail={detail} /></div><div className="min-w-0"><div className="mb-3 flex flex-wrap gap-2"><Badge className="border-white/20 bg-white/10 text-white">{title(detail.asset.structureLevel)}</Badge><Badge className={cn("border-white/30", statusTone(detail.asset.status))}>{title(detail.asset.status)}</Badge></div><p className="text-sm font-bold tracking-wide text-blue-100">{detail.asset.code}</p><h1 className="mt-1 !text-white text-2xl font-bold md:text-3xl">{detail.asset.name}</h1><p className="mt-2 flex items-center gap-2 text-sm text-blue-100"><MapPin className="size-4" /> {detail.asset.location} · {detail.asset.typeName}{detail.asset.categoryName ? ` · ${detail.asset.categoryName}` : ""}</p></div><div className="flex flex-wrap gap-2 md:max-w-44 md:justify-end"><AssetQrDialog assetId={detail.asset.id} assetCode={detail.asset.code} />{permissions.includes("ASSET_UPDATE") && <Button asChild variant="outline" className="min-h-11 border-white/40 bg-white/10 text-white hover:bg-white hover:text-[#0b2a4a]"><Link href={`/assets/${detail.asset.id}/edit`}><Pencil className="size-4" />Edit</Link></Button>}{permissions.includes("ASSET_ARCHIVE") && detail.asset.status !== "RETIRED" && <Button type="button" variant="destructive" className="min-h-11 border border-red-300" onClick={() => setConfirmArchive(true)}><Archive className="size-4" />Archive</Button>}</div></div></section>
    {archiveError && <Alert variant="destructive">{archiveError}</Alert>}
    ...
    </Tabs><ConfirmDialog open={confirmArchive} onOpenChange={setConfirmArchive} title={`Archive ${detail.asset.code}?`} description="This is a non-destructive removal. The asset and its history remain available with Retired status. Active child assets must be moved or archived first." confirmLabel="Archive asset" pending={archiving} onConfirm={archive} />
  </div>;
}

export default function AssetWorkspace({ initialAssetId, initialTab = "general", permissions = [] }: { initialAssetId?: string; initialTab?: string; permissions?: string[] }) {
  const permitted = permissions.includes("ASSET_READ");
  ...
  return <main className="mx-auto w-full max-w-[112rem] space-y-4 p-3 sm:p-4 md:p-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-700">Asset Management</p><h1 className="mt-1 text-2xl font-bold text-slate-950 md:text-3xl">Asset register</h1><p className="mt-1 text-sm text-slate-600">Systems, equipment, components, and their maintenance context.</p></div><div className="flex flex-wrap gap-2">{permissions.includes("ASSET_CREATE") && <Button asChild className="min-h-11"><Link href="/assets/new"><Plus className="size-4" />Add asset</Link></Button>}{data && <Sheet><SheetTrigger asChild><Button variant="outline" className="min-h-11 lg:hidden"><Menu className="size-4" /> Browse assets</Button></SheetTrigger><SheetContent side="left" className="w-[92vw] max-w-md p-0"><SheetHeader className="sr-only"><SheetTitle>Asset register</SheetTitle><SheetDescription>Choose an asset from the hierarchy or list.</SheetDescription></SheetHeader><AssetNavigator data={data} selectedId={selectedId} onSelect={select} mode={mode} onMode={setMode} /></SheetContent></Sheet>}</div></header>
    <Filters query={query} setQuery={setQuery} filters={filters} setFilter={(key, value) => setFilters((current) => ({ ...current, [key]: value }))} data={data} />
    {listError ? <StatePanel kind={listError === "PERMISSION" ? "permission" : "error"} title={listError === "PERMISSION" ? "Asset access required" : "Assets could not be loaded"} message={listError === "PERMISSION" ? "Your session does not include ASSET_READ." : listError} retry={() => void retryList()} /> : !data ? <div className="grid gap-4 lg:grid-cols-[22rem_1fr]"><Skeleton className="hidden h-[70vh] rounded-2xl lg:block" /><DetailSkeleton /></div> : data.assets.length === 0 ? <StatePanel kind="empty" title="No assets registered" message="The asset register is ready, but it does not contain any migrated or newly created records yet." /> : <div className="grid min-h-[70vh] gap-4 lg:grid-cols-[22rem_minmax(0,1fr)]"><aside className="sticky top-20 hidden h-[calc(100vh-6rem)] overflow-hidden rounded-2xl border bg-white shadow-sm lg:block"><AssetNavigator data={data} selectedId={selectedId} onSelect={select} mode={mode} onMode={setMode} /></aside><section className="min-w-0">{detailError ? <StatePanel kind={detailError === "PERMISSION" ? "permission" : "error"} title={detailError === "PERMISSION" ? "Asset access required" : "Asset could not be loaded"} message={detailError} retry={() => { if (selectedId) { setDetailError(""); setSelectedId(null); window.setTimeout(() => setSelectedId(selectedId), 0); } }} /> : detail ? <AssetDetail detail={detail} allAssets={data.assets} tab={tab} onTab={changeTab} permissions={permissions} /> : <DetailSkeleton />}</section></div>}
  </main>;
}
