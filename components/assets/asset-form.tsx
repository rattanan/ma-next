...
function Field({ id, label, error, wide, helpDescription, helpExample, children }: { id: string; label: string; error?: string; wide?: boolean; helpDescription?: string; helpExample?: string; children: React.ReactNode }) { return <div className={wide ? "md:col-span-2 lg:col-span-2" : ""}><Label htmlFor={id} helpDescription={helpDescription} helpExample={helpExample} className="mb-2 block">{label}</Label>{children}{error && <p className="mt-1 text-sm text-red-700" role="alert">{error}</p>}</div>; }
...
function CustomField({ field, register }: { field: Reference["customFields"][number]; register: ReturnType<typeof useForm<Values>>["register"] }) { const id = `custom-${field.id}`; const registration = register(`customFields.${field.id}`); const example = field.placeholder || field.defaultValue || options(field.availableValues)[0] || (field.fieldType === "NUMBER" ? "100" : field.fieldType === "DATE" ? "2026-09-05" : "ค่าตามข้อมูลจริงของสินทรัพย์"); return <Field id={id} label={`${field.groupName} · ${field.label}${field.unit ? ` (${field.unit})` : ""}`} helpDescription={field.description || `ข้อมูลเพิ่มเติมของหมวด ${field.groupName}`} helpExample={`เช่น ${example}`}><div>{!field.active ? <Input id={id} readOnly className="bg-slate-50 text-slate-600" {...registration} /> : field.fieldType === "ARRAY" ? <Select id={id} {...registration}><option value="">{field.placeholder || "Select value"}</option>{options(field.availableValues).map((item) => <option key={item} value={item}>{item}</option>)}</Select> : <Input id={id} type={field.fieldType === "NUMBER" ? "number" : field.fieldType === "DATE" ? "date" : "text"} placeholder={field.placeholder ?? field.defaultValue ?? undefined} {...registration} />}{field.description && <p className="mt-1 text-xs text-slate-500">{field.description}{!field.active ? " · Retired definition retained for compatibility" : ""}</p>}</div></Field>; }
