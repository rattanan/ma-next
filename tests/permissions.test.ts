...
describe("role-based access control", () => {
  ...
  it("lets every authenticated role read assets but reserves asset mutations", () => { for (const role of ["ADMIN", "DATA_SOURCE_CREATOR", "DASHBOARD_CREATOR", "VIEWER"] as const) expect(hasPermission(role, "ASSET_READ")).toBe(true); expect(hasPermission("ADMIN", "ASSET_ARCHIVE")).toBe(true); expect(hasPermission("DATA_SOURCE_CREATOR", "ASSET_HIERARCHY_MANAGE")).toBe(true); expect(hasPermission("VIEWER", "ASSET_UPDATE")).toBe(false); });
});
