...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json({ user: { id: user.id, fullName: user.fullName, username: user.username, email: user.email, role: user.role, mustChangePassword: user.mustChangePassword }, redirectTo: user.mustChangePassword ? "/change-password" : "/dashboard" });
