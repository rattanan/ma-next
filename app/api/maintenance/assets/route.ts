...
export async function GET(request: NextRequest) { const meta = getRequestMeta(request); try { const session = await requirePermission(request, "VIEW_MAINTENANCE"); return NextResponse.json({ assets: (await listMaintenanceOverview(session.user)).assets }); } catch (error) { return apiError(error, meta.requestId); } }
